...
void error(vector<print> &passfunc,int stop){
    ...
    for(int i=0;i<passfunc.size();i++){
        if(passfunc[i].second=="ldc"||passfunc[i].second=="data"||passfunc[i].second=="adc"||passfunc[i].second=="ldl"||passfunc[i].second=="stl"||passfunc[i].second=="ldnl"||passfunc[i].second=="stnl"||passfunc[i].second=="adj"||passfunc[i].second=="call"||passfunc[i].second=="brz"||passfunc[i].second=="brlz"||passfunc[i].second=="br"){
            if(!check(passfunc[i].third)){
               e=hexadecimal(i);
               e=e+" ";
               e=e+" not a number";
...
int stop;
    ...
    for (int i = 0; i < passfunc.size(); i++) {
        ...
        // Increment address
        if (passfunc[i].second == "data") {
            // Increment address only if the mnemonic is 'data'

            memory.insert({passfunc[i].first, passfunc[i].third});
        }
...
int main() {
    ...
    while (getline(infile, line)) {
        ...
        if (!label.empty() && !value.empty() && mnemonic=="data" && mnemonic=="SET") {
